...
# 💡 시도하고 싶은 개선사항
## 1. 날짜 선택 상태 유지
문제: 새로고침 시 선택한 날짜 범위가 초기화됨
개선 방향:

- URL 쿼리 파라미터에 날짜 저장
- 또는 localStorage 활용
## 2. 페이지네이션 UX 개선
문제: 페이지 수가 많아지면 네비게이션이 어려움
개선 방향:

- "처음/마지막 페이지로" 버튼 추가
- 현재 페이지 주변만 표시 (예: 1 ... 5 6 7 ... 20)
- 페이지 직접 입력 기능
## 3. 대시보드 가시성 개선
문제: 중요한 정보가 명확히 드러나지 않음
개선 방향:

- 핵심 지표 카드 추가 (총 구매 금액, 평균 구매액, 상위 고객 등)
- 차트 시각화 (막대 그래프, 파이 차트)
- 색상으로 중요도 구분 (높은 구매액 하이라이트)
